...
@Getter
@Setter
public class ShortStraddle extends OptionsStrategy {
	...
	private List<ShortStraddleRow> spotlist = new ArrayList<>();
	private String disp = null;
	private Double spotat = 0.0;
	...
	private void generate() {
		...
		sintlist.forEach(s -> {
			...
			ShortStraddleRow spot = ShortStraddleRow.builder().spotprice(s).call(callrow).put(putrow).build();
			spotlist.add(spot);
		});

	//	List<ShortStrangleOutput> soplist = new ArrayList<>();

		for (ShortStraddleRow spot : spotlist) {
...
//			ShortStrangleOutput sop = ShortStrangleOutput.builder().spot(spot.getSpotprice()).calldes(call.getDes())
//					.callAya(call.getAya()).callDiya(call.getDiya()).callPandL(call.getPandl()).putdes(put.getDes())
//					.putAya(put.getAya()).putDiya(put.getDiya()).putPandL(put.getPandl()).spotPandL(spot.getPandl())
//					.spotPandLPerLot(spot.getPlPerLot()).roi(spot.getRoi()).build();
//			soplist.add(sop);
...
//		dumper.dump("ShortStrangle");
	...
	public ShortStraddle(String str) {
		this.disp = str;
	}
	...
	public void evaluate() {
		...
		System.out.println("=========================================================================================");
		sellcall = SellCall.builder().aya(Double.valueOf(callprice)).diya(0.0).strike(Double.valueOf(callstrike))
				.build();
